...
angular.module('axismakerApp')
  .controller('NewCtrl', function ($scope, Auth, $compile, $window, $modal, $templateCache) {
    ...
    var createNew = function(config) {
      if ($scope.filename !== '') {
        var template = $templateCache.get('/app/preview/preview.html');
        repo.write($scope.branch, $scope.filename + '/index.html', template, 'initial -- ' + $scope.filename, function(err, res, xmlhttprequest){
          ...
          var url = 'https://rawgit.com/' + $scope.repoName[1] + '/' + $scope.repoName[2] + '/master/index.html';
          var cdnUrl = 'https://cdn.rawgit.com/' + $scope.repoName[1] + '/' + $scope.repoName[2] + '/master/index.html';
